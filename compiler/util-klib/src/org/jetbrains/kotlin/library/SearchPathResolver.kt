    ...
    class SearchRoot(val searchRootPath: File, val allowLookupByRelativePath: Boolean = false, val isDeprecated: Boolean = false) {
        ...
        companion object {
            fun lookUpByAbsolutePath(absoluteLibraryPath: File): File? {
                // First, try to find a KLIB file.
                val klibFile = File(absoluteLibraryPath.path.suffixIfNot(KLIB_FILE_EXTENSION_WITH_DOT))
                if (klibFile.isFile) return klibFile

                // Then, check if it's a directory.
                val klibDir = File(absoluteLibraryPath.path.removeSuffixIfPresent(KLIB_FILE_EXTENSION_WITH_DOT))
                if (klibDir.isDirectory) return klibDir
